...
- **FPS Guard（原始帧率超阈值自动收场）** —— 帧生成运行期间轮询原始帧率，
  高于自定义阈值（默认 62，首页可设 30–240）即自动结束会话，避免菜单画面 /
  90–120Hz 帧源的垃圾读数被拿去插帧。仅 Shizuku/Root 捕获模式生效。
- **HDR 内容检测（已移除应用内 HDR 模式）** —— 屏幕出现 HDR10 / 杜比视界 / HLG
  内容时，SurfaceFlinger 会把我们的 SDR 悬浮层踢出合成，悬浮窗方案在 HDR 内容下
  无法存活，因此检测到即自动结束会话并提示（约 3 秒）。上游式的应用内
  "HDR 模式"开关已移除——它救不了这条链路。仅 Shizuku/Root 模式生效
  （MediaProjection 模式回退到下方看门狗兜底）。
- **输出停滞看门狗** —— 悬浮层被踢出合成、`vkAcquire` 无限超时（已知诱因是
  HDR 色域切换）时，约 5 秒内判定输出已死并优雅结束会话，不再冻屏到被系统
  低内存清理杀掉进程。
- **稳定性修复** —— 会话结束后偶发的僵尸悬浮窗吃掉全部触摸（移除失败改为
  带日志重试并持续清空触摸区域）、CPU 回退 blit 路径的 SIGSEGV 竞态、
  连续 acquire 超时后的 swapchain 重建、ImageReader 回调崩溃，全部修复。
...
- **游戏内实时设置抽屉**：倍率（2×–8×）、flow scale（0.25–1.0）、性能模式、
