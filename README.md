# LSFG-Android-Plus — frame generation on Android via the lsfg-vk pipeline

[![Discord](https://img.shields.io/discord/1496212333595463780?label=Discord&logo=discord)](https://discord.gg/CkuumNJ7s4)

> **A Chinese-localized & enhanced fork of [FrankBarretta/LSFG-Android](https://github.com/FrankBarretta/LSFG-Android).**
> 简体中文说明请看 [README_zh-CN.md](README_zh-CN.md)。
...
## What's different in this fork (Plus)

Everything from upstream works as before; on top of it, this fork adds:

- **Requires Android 14 or newer** — minSdk raised from 29 (Android 10) to
  34 (Android 14); the privileged capture / timing paths target modern
  Android 14–16 releases (ColorOS included). Older Android versions are
  not supported.
- **Simplified-Chinese localization + English i18n** — ~125 previously
  hard-coded UI strings extracted to resources. The app follows the system
  language: Chinese systems get Chinese, everything else gets English.
- **Pause frame generation on touch** — hold or tap the screen while the
  overlay is up and frame generation pauses until you let go. Shizuku/Root
  capture modes listen to `/dev/input` for precise touch detection (long-press
  or tap both work); MediaProjection mode falls back to a 1 px transparent
  window that catches touches outside the overlay (tap only). **Must be
  disabled while gaming** — otherwise every touch would pause frame
  generation mid-game.
- **Millisecond-level overlay recovery after rotation** — the native render
  loop now aborts immediately when a stop is requested instead of draining
  queued frames with 500 ms acquire timeouts, and framegen persists its
  compute pipelines to a `VkPipelineCache` file (`framegen_pcache_<uuid>.bin`)
  so the ~2 s first-compile cost is paid once, not on every re-init.
- **Auto session end in freeform / floating windows** — a watcher detects
  when the target app's window leaves fullscreen (window bounds ≠ display
  max bounds, the reliable signal on ColorOS) and ends the session with a
  toast, since UID-filtered capture can only show the target window anyway.
- **Overlay-permission entry on the home screen** — a green/orange permission
  chip with a one-tap jump to the system overlay-permission page, plus an
  automatic prompt on first launch when the permission is missing.
- **Home-screen rework** — capture mode and pacing presets moved to the home
  screen as selectable chips; redundant info cards removed; switching the
  frame multiplier no longer freezes the overlay (re-init now falls back to
  the active target package).
- **FPS Guard** — when frame generation is running, a guard polls the source
  frame rate and ends the session automatically if it exceeds a customizable
  threshold (default 62, configurable 30–240 on the home screen), so junk
  readings from menu screens / 90–120 Hz sources don't get interpolated.
  Shizuku/Root capture modes only.
- **HDR content detection (in-app HDR mode removed)** — HDR10 / Dolby Vision /
  HLG content on screen makes SurfaceFlinger drop our SDR overlay from
  composition; frame generation cannot survive HDR content with the overlay
  approach, so the session ends automatically with a toast (~3 s). The
  upstream-style in-app "HDR mode" toggle was removed as it cannot help.
  Shizuku/Root capture modes only (MediaProjection falls back to the watchdog
  below).
- **Output-stall watchdog** — if the overlay is dropped from composition and
  `vkAcquire` times out indefinitely (HDR color-mode switch being the known
  trigger), the app detects the dead output within ~5 s and ends the session
  gracefully instead of freezing until the system low-memory killer reaps it.
- **Stability fixes** — a zombie overlay window that swallowed all touch input
  after a session ended (removal now retries with the touchable region kept
  cleared), a SIGSEGV race in the CPU fallback blit path, swapchain recreation
  after repeated acquire timeouts, and an ImageReader callback crash are all
  fixed.
...
| Path | What it is |
|---|---|
| [`LSFG-Android-Application/`](LSFG-Android-Application/) | Android Studio project — Kotlin + Jetpack Compose UI, JNI/C++ render loop. The user-facing app. |
...
- **Live in-game settings drawer**: multiplier (2×–8×), flow scale (0.25–1.0),
  performance mode, anti-artifacts, bypass, vsync alignment with slack
...
`minSdk=34` (Android 14), `targetSdk=35` (Android 15). **Android 14 or newer is required.**
...
- [`LSFG-Android-Application/README.md`](LSFG-Android-Application/README.md) —
  app architecture, feature breakdown, device requirements, native module layout.
...
- **[FrankBarretta](https://github.com/FrankBarretta)** — Android port
  (upstream repository): JNI/Vulkan glue, AHardwareBuffer-based image sharing,
...
|---|---|---|
| [`LSFG-Android-Application/`](LSFG-Android-Application/) | **Custom License — No Play Store, No Commercial Use** | [`LSFG-Android-Application/LICENSE`](LSFG-Android-Application/LICENSE) |
